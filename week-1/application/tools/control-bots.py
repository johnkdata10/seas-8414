...
import freqtrade_client as fc
import port_map

servers = port_map.servers


def get_client_handle(server=None, port=8001, config="config/freqtrade-bots.json"):
    config = fc.load_config(config)
    url = server if server else config.get('api_server', {}).get('listen_ip_address', '127.0.0.1')
    port = port if port else config.get('api_server', {}).get('listen_port', '8080')
    username = config.get('api_server', {}).get('username')
    password = config.get('api_server', {}).get('password')
    server_url = f"http://{url}:{port}"
    client = fc.FtRestClient(server_url, username, password)
    return client
...
def main(args):
    ...
    for server in servers:
        for strat in servers[server]:
            port = servers[server][strat]
            # print(server, strat, port, flush=True)
            client = get_client_handle(server=server, port=port)
            ...
            record = getattr(client, command)(*args["command_arguments"])
            if command == "status":
                is_status = True
                if len(results) == 0:
                    results = record
                else:
                    for r in record: results.append(r)
            else:
                record["strategy"] = strat
                record["strat_server"] = server
                record["port"] = port
    ...
    if is_status:
        # print(df.columns)
        cols = ['strategy', 'pair', 'is_short', 'stake_amount',  'open_date', 'open_rate', 'current_rate',
                'profit_pct', 'profit_abs', 'trading_mode', 'current_profit', 'current_profit_abs',
                'current_profit_pct']
        print(df[cols])
    else:
        print(df)
